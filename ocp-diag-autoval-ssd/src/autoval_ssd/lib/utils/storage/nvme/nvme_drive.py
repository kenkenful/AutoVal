...
class NVMeDrive(Drive):
    ...
    @property
    def generic_name(self, nvme_list_info=None) -> str:
        """
        Get the generic name of the drive.

        Returns:
            The generic name eg ng1n1
        """
        return NVMeUtils.get_from_nvme_list(
            self.host, self.block_name, "Generic", nvme_list_info
        )
    ...
    def get_target_path() -> str:
        ...
        try:
            ...
            if match:
                target_path = match.group(0).removesuffix("/lib")
        except Exception:
    ...
    def get_firmware_version(self, nvme_list_info=None):
        """Return drive FW version"""
        return NVMeUtils.get_from_nvme_list(
            self.host, self.block_name, "Firmware", nvme_list_info
        )
    ...
    def get_smart_log(self):
        ...
        try:
            ...
            log = NVMeUtils.parse_json_string(output)
    ...
    def get_ocp_smart_log(self) -> dict:
        ...
        try:
            ...
            log = NVMeUtils.parse_json_string(out)
            result = GenericUtils.flatten_dict(log)
        except Exception:
            AutovalLog.log_info(
    ...
    def collect_data(self, nvme_list_info=None):
        data = {
            "SMART": self.get_smart_log(),
            "firmware": self.get_firmware_version(nvme_list_info),
            "serial_number": self.serial_number,
    ...
    def get_power_mode(self, selection: int | None = None) -> int:
        """
        This method gets the power mode (defaults to current if no
        selection provided) using the power management feature of
        the drive.

        Args:
            selection: The selection parameter to be passed to nvme get-feature

        Returns:
            Power mode of the drive returned from nvme get-feature.
        """
        cmd = f"nvme get-feature /dev/{self.block_name} -f 0x2"
