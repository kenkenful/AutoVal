...
class NVMeUtils:
    ...
    def get_from_nvme_list(host, block_name, field, nvme_list_info=None):
        ...
        try:
            ...
            raise TestError(
                "Unable to find DevicePath for {} in {}".format(
                    block_name, nvme_list_info
                )
            )
        if field not in drive_data:
            raise TestError("Unable to find {} in {}".format(field, drive_data))
            ...
            if "Namespaces" in controller and controller["Namespaces"]:
                for namespace_info in controller["Namespaces"]:
                    ...
                    old_format = {
                        ...
                        "Generic": namespace_info.get("Generic", ""),
                    }
